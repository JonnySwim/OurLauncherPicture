...
//---- Map a KeyEvent to a string that describes the keys pressed
	...
	var aliasgenerator = {
		...
		point: '.',
		lt: '<',
		'f(\\d+)': 'F$1', // uppercase function keys
...
//---- Modify KeyEvents to use key descriptions				
	...
	["keydown","keyup"].forEach(function(type){
		...
		$.event.special[type].handle = function (evt){
			...
			var keys = evt.hasOwnProperty('data') && evt.data ? (evt.data.hasOwnProperty('keys') && evt.data.keys ? evt.data.keys : null) : null; // array of target keys for this handler
